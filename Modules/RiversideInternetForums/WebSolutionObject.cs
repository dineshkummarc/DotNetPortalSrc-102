...
namespace RiversideInternet.WebSolution
{
	public class WebSolutionObject
	{
		...
		{
			string configValue = ConfigurationSettings.AppSettings["RiversideInternetForumsImagesURL"];
			if (configValue != null)
			{
		...
		{
			string configValue = ConfigurationSettings.AppSettings["RiversideInternetForumsAvatarsURL"];
			if (configValue != null)
			{
				if (configValue.IndexOf("{0}") >= 0)
					configValue = string.Format(configValue, WebFolder);
		...
		{
			string avatarPath = ConfigurationSettings.AppSettings["RiversideInternetForumsAvatarsURL"];
			if (avatarPath != null)
			{
...
/*		protected string GetUserManagement(string action)
		{
			string configValue = ConfigurationSettings.AppSettings["WebSolutionUserManagementURL"];
			if (configValue != null)
			{
